...
class Sim {
  ...
  issueCredential(id: Uint8Array, metadata: string): Ledger {
    this.ctx = this.contract.impureCircuits.issueCredential(this.ctx, id, metadata).context;
    return this.getLedger();
  }

  verifyCredential(providedId: Uint8Array): Ledger {
    this.ctx = this.contract.impureCircuits.verifyCredential(this.ctx, providedId).context;
    return this.getLedger();
  }
  ...
  authorityPublicKey(): Uint8Array {
    const seqBytes = convertFieldToBytes(32, this.getLedger().sequence, 'test-suite');
...
describe('Circuit Logic', () => {
  it('should set credentialState to ACTIVE after issueCredential', () => {
    ...
    const state = sim.issueCredential(credId, 'CredShield University Diploma 2025');
...
describe('State Transitions', () => {
  ...
  it('should reject issueCredential when credential is already ACTIVE (double-issue guard)', () => {
    ...
    expect(() => sim.issueCredential(randomBytes(32), 'Second Credential')).toThrow();
  });
});
...
describe('Privacy', () => {
  ...
  it('should keep secretKey only inside local private state — never on-chain', () => {
    ...
    expect(toHex(onChainState.issuerAuthority)).not.toBe(toHex(privateState.secretKey));
  });
